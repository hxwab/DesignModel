 ...
 * @author win7
 * 
 * @category 增加了数据库访问管理类DataAccess类，删除了工厂类。如果在增加一个数据
 *      	 表，只须增加Access和SqlServer的相应类，并且在DataAccess中增加判断语句
 *        
 *@problem  如果还要增加一个数据库Oracle,在抽象工厂中要增加Oracle工厂类及子类，这是增加类，不可避免
 *       。（开放原则）
 *		      在简单工厂中创建实例则要在DataAccess中的swicth 中增加判断语句（违背了封闭原则）
 *
 *@solution 利用反射机制解决实例化的问题，根据DB的不同直接返回创建对象
 *			见version 4.0
 *
 */
